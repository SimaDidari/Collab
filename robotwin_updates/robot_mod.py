...
# NEW: function to find link by name
def find_link_by_name(entity, name):
...
class Robot:
    ...
    def _init_robot_(self, scene, need_topp=False, **kwargs):
        ...
        # NEW: adding info about the left arm  links
        self.left_arm_links_name = left_embodiment_args["arm_links_name"][0]
        self.left_arm_baselink_name = left_embodiment_args["arm_base_links_name"][0]
        ...
        # NEW: added info for right arm link
        self.right_arm_links_name = right_embodiment_args["arm_links_name"][1]
        self.right_arm_baselink_name = right_embodiment_args["arm_base_links_name"][1]
    ...
    def print_info(self):
        ...
        # NEW
        print("left arm links: ", [link.get_name() for link in self.left_arm_links])
        print("right arm links: ", [link.get_name() for link in self.right_arm_links])
    ...
    def get_left_arm_linkPose(self) -> list:
        linkPoseList = []
        for link in self.left_arm_links:
            pose = link.get_pose()
            linkPoseList.append(pose.p.tolist() + pose.q.tolist())  # [x, y, z, qw, qx, qy, qz]
        return linkPoseList
    ...
    def get_right_arm_linkPose(self) -> list:
        linkPoseList = []
        for link in self.right_arm_links:
            pose = link.get_pose()
            linkPoseList.append(pose.p.tolist() + pose.q.tolist())  # [x, y, z, qw, qx, qy, qz]
        return linkPoseList
