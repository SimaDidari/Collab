...
def load_hdf5(dataset_path):
    ...
    with h5py.File(dataset_path, "r") as root:
        ...
        # --- NEW ---
        left_arm_poses = root["/links_pose/left_arm"][()]   # shape (T, num_links, 7)
        right_arm_poses = root["/links_pose/right_arm"][()]  # shape (T, num_links, 7)
        dual_arm_poses = root["/links_pose/dual"][()]    # shape (T, total_num_links, 7)
        # -----------
    ...
    return left_gripper, left_arm, right_gripper, right_arm, vector, pointcloud, image_dict, left_arm_poses, right_arm_poses, dual_arm_poses
...
def main():
    ...
    while current_ep < num:
        ...
        (
            ...
            pointcloud_all,
            image_dict_all,
            left_arm_poses_all,   # NEW
            right_arm_poses_all,  # NEW
            dual_arm_poses_all,    # NEW
        ) = load_hdf5(load_path)
        ...
        for j in range(0, left_gripper_all.shape[0]):
            ...
            link_state = dual_arm_poses_all[j]
            ...
            if j != left_gripper_all.shape[0] - 1:
                ...
                # --- NEW ---
                links_state_arrays.append(link_state)
                # -----------
            if j != 0:
                joint_action_arrays.append(joint_state)
                # --- NEW ---
                links_action_arrays.append(link_state)
                # -----------
    ...
    try:
        ...
        head_camera_arrays = np.moveaxis(head_camera_arrays, -1, 1)  # NHWC -> 
        # --- NEW ---- 
        links_state_arrays = np.array(links_state_arrays)
        links_action_arrays = np.array(links_action_arrays)
        #------------
        ...
        zarr_data.create_dataset(
            "links_state",
            data=links_state_arrays,
            chunks=(100, *links_state_arrays.shape[1:]),
            dtype="float32",
            overwrite=True,
            compressor=compressor,
        )
        zarr_data.create_dataset(
            "links_actions",
            data=links_action_arrays,
            chunks=(100, *links_action_arrays.shape[1:]),
            dtype="float32",
            overwrite=True,
            compressor=compressor,
        )
