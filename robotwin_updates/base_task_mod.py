...
class Base_Task(gym.Env):
    ...
    def get_obs(self):
        ...
        # NEW: adding the arm links pose information
        if self.data_type.get("links_pos", False):
            left_links_pos = self.robot.get_left_arm_linkPose()
            right_links_pos = self.robot.get_right_arm_linkPose()
            pkl_dic["links_pose"]["left_arm"] = left_links_pos 
            pkl_dic["links_pose"]["right_arm"] = right_links_pos
            both_arms_pos = np.concatenate([np.array(left_links_pos), np.array(right_links_pos)], axis = 0)
            pkl_dic["links_pose"]["dual"] = both_arms_pos.tolist()
