            ...
            for batch_start in range(0, len(data), self.batch_size):
                ...
                if len(logprobs) > 0:
                    ...
                    if loss.requires_grad:
                        ...
                        logger.info(f"Repetition {rep}: Batch {batch_start // self.batch_size + 1} score: {scores_tensor.cpu().tolist()}")
                        logger.info(f"Repetition {rep}: Batch {batch_start // self.batch_size + 1} cost: {costs_tensor.cpu().tolist()}")
                    else:
